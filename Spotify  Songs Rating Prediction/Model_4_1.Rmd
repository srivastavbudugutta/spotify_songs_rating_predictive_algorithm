...
create_vocal_dominance = function(dat){
  dat = dat %>% mutate(voice_dominance = 1 - instrumentalness - speechiness)
  return(dat)
}

create_loudness_energy = function(dat){
  dat = dat %>% mutate(loudness_energy = loudness/energy)
  return(dat)
}


create_speechiness = function(dat){
  dat = dat %>% mutate(speechiness_value = speechiness * energy)
  return(dat)
}

create_emotional_intensity = function(dat){
  dat = dat %>% mutate(emotional_intensity = (loudness / -60) * valence)
  return(dat)
}

create_valence_tempo = function(dat){
  dat = dat %>% mutate(valence_ratio = valence/tempo)
  return(dat)
}
...
sapply(songs, function(data) which(is.na(data)))
# songs = apply(songs,2,max,na.rm=TRUE)
which(is.na(songs))
# dim(songs)
# which(is.na(songs))
# songs = impute_values(songs)
# which(is.na(songs))
# songs
# dim(songs)
...
# there
geners_modify_2 = create_emotional_intensity(geners_modify_2)
# there
geners_modify_2 = create_vocal_dominance(geners_modify_2)
# there
geners_modify_2 = create_valence_tempo(geners_modify_2)

# there
geners_modify_2 = create_speechiness(geners_modify_2)
# there
geners_modify_2 = create_loudness_energy(geners_modify_2)
geners_modify_2 = impute_values(geners_modify_2)
which(is.na(geners_modify_2))
```
...
songs_test_2 = create_instrumental_energy(songs_test_2)
# there
songs_test_2 = create_emotional_intensity(songs_test_2)
# there
songs_test_2 = create_vocal_dominance(songs_test_2)
# there
songs_test_2 = create_valence_tempo(songs_test_2)
# there
songs_test_2 = create_speechiness(songs_test_2)
# there
songs_test_2 = create_loudness_energy(songs_test_2)
...
start_mod = lm(rating~1,data=geners_modify_2)
empty_mod = lm(rating~1,data=geners_modify_2)

full_mod = lm(rating~performer+tempo+time_signature+mode+speechiness+ track_explicit+genre+track_duration+danceability+energy+key+loudness+ interactive+song_intensity+acoustic_energy+instrumental_energy+liveenergy+
                valence_ratio+
                emotional_intensity+
                speechiness_value+
                loudness_energy+
                voice_dominance,geners_modify_2)
...
model_1 = lm(rating~liveenergy+
               ...
               interactive+
               song_intensity+
               track_explicit,geners_modify_2)
...
write.csv(submissionFile, 'model_4_1.csv',row.names = F)
# head(songs_test)
# head(geners_modify)
# dim(geners_modify)
